...
5. Tool Selection:
   - Long-Running Processes: Start watchers (e.g. bun test --watch, cargo watch) and dev servers (npm run dev, vite) with `shell` and `background: true`; never block on a process that does not exit. Give long builds and test suites a longer `timeout`.
